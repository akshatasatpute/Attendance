...
def generate_random_number():
    return random.randint(1, 100)  # Generate a random number between 1 and 100 inclusive

entered_key = st.text_input('Enter Specific Key',type='password')

if entered_key == specific_key:
    st.write('Specific Key verified!')

    random_number = generate_random_number()  # Retrieve the random number from the cache
    st.write('Random Number:', random_number)

    access_code = st.text_input("Enter Access Code")
    if access_code:
        try:
            access_code = int(access_code)  # Convert the input to an integer for comparison
            if access_code == random_number:
                st.write('Access Code correct! You can access the Google form.')
            else:
                st.write('Access Code incorrect. Please try again.')
        except ValueError:
